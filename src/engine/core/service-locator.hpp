...
class ServiceLocator {
  private:
    ...
    static inline std::unordered_map<std::type_index,
                                     std::unique_ptr<ServiceContainerBase>>
        s_services;
  ...
  public:
    ...
    template <typename T> static T &get() {
        ...
        assert(it != s_services.end() &&
               "Requested service was never provided!");
    ...
    template <typename T> static void remove() {
        ...
        auto it = std::find(s_registration_order.begin(),
                            s_registration_order.end(), typeid(T));
    ...
    static inline void shutdown() {
        for (auto it = s_registration_order.rbegin();
             it != s_registration_order.rend(); ++it) {
            s_services.erase(*it);
        }
